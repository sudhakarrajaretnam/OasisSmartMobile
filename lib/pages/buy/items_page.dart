...
// import 'package:oasis_smart_services/pages/buy/footer/cart_footer.dart';
...
class ItemsPage extends ConsumerStatefulWidget {
  ...
  final String categiryId;
  const ItemsPage(
      {super.key, required this.categoryName, required this.categiryId});
  ...
  Widget build(BuildContext context) {
    ...
    final allItems = ref.watch(
      groceryItemsProvider(
          GroceryItemsParams(categoryId: categoryId, query: '')),
    );
    final cart = ref.watch(cartProvider);
    return Scaffold(
      appBar: PreferredSize(
          ...
          actions: [
            Container(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  final searchFlag =
                      ref.read(showSearchNotifier.notifier).state;
                  ref.read(showSearchNotifier.notifier).state = !searchFlag;
                  if (!searchFlag) {
                    Future.delayed(const Duration(milliseconds: 300), () {
                      _focusNode.requestFocus();
          ...
          Expanded(
            child: Padding(
              ...
              child: allItems.when(
                data: (items) {
                  ...
                  return GridView.builder(
                    gridDelegate:
                      ...
                      mainAxisSpacing: 8.0,
                      //childAspectRatio: 0.68, // Adjust the height of each card
                      childAspectRatio: 0.6, // Adjust the height of each card
                    ),
                    itemCount: items.length,
                    itemBuilder: (context, index) {
                      final item = items[index];
                                  ...
                                  child: Align(
                                    ...
                                    child: CachedNetworkImage(
                                      ...
                                      placeholder: (context, url) =>
                                        ...
                                        child: SizedBox(
                                          width:
                                              24, // Width of the circular indicator
                                          height:
                                              24, // Height of the circular indicator
                                          child: CircularProgressIndicator(
                                            strokeWidth:
                                                2.0, // Thickness of the progress indicator
                                          ),
                                      ...
                                      errorWidget: (context, url, error) =>
                                          const Icon(Icons
                                              .broken_image), // Fallback for errors
                                      fit: BoxFit.contain,
                                    ),
                                  ),
                                ),
                                        ...
                                        child: Material(
                                          ...
                                          child: InkWell(
                                            ...
                                            child: Ink(
                                              ...
                                              decoration: const BoxDecoration(
                                                color: Color(
                                                    0xFFE9F6E1), // Green color for the button
                                                borderRadius:
                ...
                error: (error, stackTrace) => Center(
                  child: Text(
                      'Error: ${error.toString()}'), // Show error message if API fails
                ),
              ),
            ),
          ),
